...
import { mapBankImportRows, money } from "../lib/app-helpers.js";
...
export function BankImportView({
  ...
  const previewRows = mapBankImportRows(bankImport.rows.slice(0, 8), bankImport.mapping, state);
      ...
      {bankImport.headers.length ? (
        <div className="grid">
          ...
          <div className="toolbar">
            <p className="notice">{bankImport.rows.length} source row(s). Previewing up to 8 converted rows.</p>
          ...
          <Table headers={["Date", "Type", "Category", "Subcategory", "Account", "Description", "Amount"]}>
            {previewRows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="money">{money(row.amount, state.currency)}</td>
              </tr>
            ))}
          </Table>
        </div>
