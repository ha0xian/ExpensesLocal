...
def _build_snapshot(state: dict, auto_status: dict | None = None) -> dict:
    ...
    # Remove full state from automation status to keep response lean
...
def create_transaction(form: dict) -> dict:
    ...
    if not (transaction["date"] and transaction["type"] and transaction["category"]
            and transaction["subcategory"] and transaction["account"] and amount):
        raise ValueError("Date, type, category, subcategory, account, and amount are required.")
...
def update_category(category_id: str, field: str, value) -> dict:
    return _update_collection("categories", category_id, field, value)
...
def update_subcategory(subcategory_id: str, field: str, value) -> dict:
    return _update_collection("subcategories", subcategory_id, field, value)
...
def update_account(account_id: str, field: str, value) -> dict:
    return _update_collection("accounts", account_id, field, value)
...
def _update_collection(collection: str, item_id: str, field: str, value) -> dict:
    state, auto_status = _load_and_automate(run_automation=False)
